...
from keras.layers import Input, Dense, Lambda, merge #Reshape
...
layers = 2
hidden_size = 100
def createLayers(name1, env):
   x = Input(shape=env.observation_space.shape, name=name1 + '_x')
   u = Input(shape=env.action_space.shape, name=name1 + '_u')
   if batch_norm:
     h = BatchNormalization()(x)
   else:
     h = x
   for i in xrange(layers):
     h = Dense(hidden_size, activation=activation, name=name1 + '_h'+str(i+1))(h)
     if batch_norm and i != layers - 1:
       h = BatchNormalization()(h)
   v = Dense(1, init='uniform', name=name1 +  '_v')(h)
   m = Dense(1, init='uniform', name=name1 + '_m')(h)
   l = Dense(1, name=name1 + '_l0')(h)
   p = Lambda(P, output_shape=(1, 1), name=name1 + '_p')(l)
   a = merge([m, p, u], mode=A, output_shape=(None,), name=name1 + "_a")
   q = merge([v, a], mode=Q, output_shape=(None,), name=name1 + "_q")
   return x, u, m, v, q
